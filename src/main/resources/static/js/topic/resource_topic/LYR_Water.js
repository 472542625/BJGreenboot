var image_LYR_Water_layer;
function add_image_LYR_Water_layer() {

	var wmsSource = new ol.source.TileWMS({
		url : server_url + 'geoserver/forest/wms',//  
		params : {
			'LAYERS' : 'forest:LYR_Water',//  
			'TILED' : false
		},
		serverType : 'geoserver'//  
	})
	image_LYR_Water_layer = new ol.layer.Tile({
		source : wmsSource,
		zIndex : 999
	});

	map.addLayer(image_LYR_Water_layer);
}

var vector_LYR_Water_layer;
function add_vector_LYR_Water_layer() {
		...
		success : function(result) {
			...
			for (var i = 0; i < features.length; i++) {
				...
				var point = new ol.Feature({
					...
					图斑面积 : features[i].sumTub,
					绿化面积 : features[i].sumLvdi,
					绿化覆盖面积 : features[i].sumlhf,
					屋顶绿化面积 : features[i].sumRofe,
					其他面积 : features[i].sumQita,
...
function addLYR_Water_layer() {

	add_image_LYR_Water_layer();
    add_vector_LYR_Water_layer();
 
}

function deleteLYR_Water_layer() {
    delete_layer(image_LYR_Water_layer);
	delete_layer(vector_LYR_Water_layer);
}